...
class Episode extends BaseModel
{
    ...
    public static function get_episode($movieId,$user_id,$profile_id,$device_id)
    {
        $query = Episode::select([
            ...
            'purchase_type','access_duration','discount','available_for','status','start_time','end_time','enable_subtitle','sno_order'
        ])
        ->with([
            'plan:id,level',
             'subtitles',
    ...
    public static function get_pay_per_view_episodes()
    {
        ...
        $query = Episode::select([
            ...
            'trailer_url', 'video_url_input', 'access', 'price','sno_order'
        ])
        ->with(['plan:id,level'])
