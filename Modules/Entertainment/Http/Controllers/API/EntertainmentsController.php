...
class EntertainmentsController extends Controller
{
    ...
    public function movieListV3(Request $request)
    {
      ...
      public function tvshowListbynetwork(Request $request)
    {
        $perPage = $request->input('per_page', 10);
    ...
    public function tvshowListV3(Request $request)
{
  ...
  $networkId = $request->query('networkId');
   ...
   ///exit;
    ...
    $tvshowList = Entertainment::query()
       ...
       if ($networkId!='null' and !empty($networkId) ) {  
 ...
 //dd($tvshowList->toSql());
    ...
    public function getNetworkListV3()
    {
        ...
        foreach ($networks as $network) {
            $network->image = !empty($network->image)
                ? asset('storage/series_networks/image/' . $network->image)
                : '';

            $network->banner_image = !empty($network->banner_image)
                ? asset('storage/series_networks/image/' . $network->banner_image)
                : '';
        }
