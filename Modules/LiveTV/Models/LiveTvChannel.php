...
class LiveTvChannel extends BaseModel
{
    ...
    protected $fillable = [
        'name','live_stream_type','slug','category_id','poster_url','thumb_url','access','plan_id','description','status','poster_tv_url'
    ];
    ...
    public static function get_top_channel($channelIdsArray)
    {
        $items = LiveTvChannel::select([
            'id','name','live_stream_type','slug','plan_id','description','status','access','category_id','poster_url','poster_tv_url',
        ])
        ->with([
            'plan:id,level',
            'TvCategory:id,name',
            'TvChannelStreamContentMappings:id,tv_channel_id,stream_type,embedded,server_url,server_url1,upcoming_date'
        ])
        ...
        return $items->map(function ($item) {
            ...
            $item->rtmp_server = $item->rtmp_server;
             $item->rtmp_key = $item->rtmp_key;
            $item->upcoming_date=$item->upcoming_date;
