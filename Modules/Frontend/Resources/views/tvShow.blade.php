...
.slide img {
    ...
    <div class="slide">
        <img src="{{ asset('storage/series_networks/image/'.  $network_banner_image) }}" alt="Banner 1">
    </div>
    ...
    <script>
       ...
       // const urlParams = new URLSearchParams(window.location.tv-shows);
   /// const networkId = urlParams.get('networkid');
   
    const urlParams = new URLSearchParams(window.location.search);
    //const networkId = urlParams.get('networkid');
    
    const networkId = <?php echo $networkId ?>;

    //console.log('networkId:', networkId);
   
        const baseUrl = document.querySelector('meta[name="baseUrl"]').getAttribute('content');

       // const apiUrl = `${baseUrl}/api/v3/tvshow-list?is_ajax=1&per_page=${per_page}&networkId=${networkId}`;
